...
class Persona(db.Model):
    ...
    )  # Sin default, viene de gore_objeto
    ...
    entidad_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("gore_actores.entidad.id")
    )  # FK a gore_actores.entidad
...
class Usuario(UserMixin, db.Model):
    ...
    persona_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("gore_actores.persona.id"), nullable=False
    )
