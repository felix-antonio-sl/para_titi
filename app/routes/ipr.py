...
def lista():
    ...
    if current_user.puede_ver_todas_ipr():
        ...
        query = Iniciativa.query.filter(
            Iniciativa.responsable_id == current_user.id
        )
    ...
    return render_template('ipr/lista.html',
...
def ficha(id):
    ...
    compromisos = iniciativa.compromisos.order_by(CompromisoOperativo.fecha_limite).all()
    alertas = iniciativa.alertas.filter(AlertaIPR.activa == True).all()
    convenios = iniciativa.convenios.all()

    return render_template('ipr/ficha.html',
                           ipr=iniciativa,
                           problemas=problemas,
...
def asignar_responsable(id):
    ...
    if responsable_id:
        ...
        iniciativa.responsable_id = UUID(responsable_id)
        from app.extensions import db
        db.session.commit()
    ...
    if request.method == 'POST':
        ...
        if not all([convenio_id, numero, periodo_desde, periodo_hasta]):
            ...
            informe = InformeAvance(
                ...
                elaborador_id=current_user.persona_id if hasattr(current_user, 'persona_id') else None,
                resumen=resumen
            )
            db.session.add(informe)
            
            # Actualizar avance en la iniciativa si se proporcionó
            ...
            flash(f'Informe de avance #{numero} registrado correctamente.', 'success')
            return redirect(url_for('ipr.ficha', id=id))
    
    # Calcular siguiente número de informe
    ultimo_informe = None
    if convenios:
        ultimo_informe = InformeAvance.query.filter(
            InformeAvance.convenio_id.in_([c.id for c in convenios])
        ).order_by(InformeAvance.numero.desc()).first()
    
    siguiente_numero = (ultimo_informe.numero + 1) if ultimo_informe else 1
    
    return render_template('ipr/registrar_avance.html',
                           ipr=iniciativa,
                           convenios=convenios,
...
def historial(id):
    ...
    for convenio in convenios:
        for informe in convenio.informes_avance.order_by(InformeAvance.numero.desc()).all():
            informes.append({
                'informe': informe,
                'convenio': convenio
            })
    ...
    return render_template('ipr/historial.html',
                           ipr=iniciativa,
                           informes=informes)
