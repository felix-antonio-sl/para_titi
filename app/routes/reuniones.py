...
from app.models.reuniones import (
    Reunion,
    TemaReunion,
    ContextoPuntoCrisis,
    InstanciaColectiva,
...
def lista():
    ...
    if not _reuniones_habilitadas():
        flash(
            "El sistema de reuniones aún no está habilitado en el modelo de datos v4.1.",
            "info",
        )
    ...
    query = Reunion.query.join(InstanciaColectiva).order_by(
        InstanciaColectiva.fecha.desc()
    )
    paginacion = query.paginate(page=page, per_page=20, error_out=False)

    return render_template(
        "reuniones/lista.html", reuniones=paginacion.items, paginacion=paginacion
    )
...
def preparar(id):
    ...
    return render_template(
        "reuniones/preparar.html", reunion=reunion, temas=temas, sugerencias=sugerencias
    )
...
def _generar_sugerencias():
    ...
    alertas = (
        AlertaIPR.query.filter(
            AlertaIPR.activa == True, AlertaIPR.nivel.in_(["CRITICO", "ALTO"])
        )
        .limit(5)
        .all()
    )
        ...
        sugerencias.append(
            {
                ...
                "iniciativa_id": (
                    str(alerta.iniciativa_id) if alerta.iniciativa_id else None
                ),
            }
        )
        ...
        sugerencias.append(
            {
                ...
                "iniciativa_id": (
                    str(comp.iniciativa_id) if comp.iniciativa_id else None
                ),
            }
        )
    ...
    completados = (
        CompromisoOperativo.query.filter(CompromisoOperativo.estado == "COMPLETADO")
        .limit(5)
        .all()
    )
    ...
    # IPR críticas
    ipr_criticas = (
        Iniciativa.query.filter(Iniciativa.nivel_alerta == "CRITICO").limit(3).all()
    )
...
def agregar_tema(id):
    ...
    max_numero = (
        db.session.query(func.max(TemaReunion.numero))
        .filter(TemaReunion.instancia_id == reunion.id)
        .scalar()
    )
...
def conducir(id):
    ...
    return render_template(
        "reuniones/conducir.html", reunion=reunion, temas=temas, usuarios=usuarios
    )
...
def ver(id):
    ...
    # Participantes (si existe tabla de asistencia, usar; sino lista vacía)
    participantes = []
    try:
        # Intento cargar participantes si la relación existe
        if hasattr(reunion.instancia, "asistentes"):
            participantes = reunion.instancia.asistentes
    except Exception:
        pass

    # Usuarios para selector de agregar participante
    usuarios = Usuario.query.filter_by(activo=True).order_by(Usuario.username).all()
...
@reuniones_bp.route("/<uuid:id>/notas", methods=["POST"])
@login_required
def guardar_notas(id):
    """Guardar notas genéricas de la reunión."""
    if not _reuniones_habilitadas():
        return render_template("reuniones/no_disponible.html")
    ...
    # Guardamos en observaciones de instancia (campo ya existente)
    reunion.instancia.observaciones = notas
    db.session.commit()
...
@reuniones_bp.route("/<uuid:id>/acta", methods=["POST"])
@login_required
def subir_acta(id):
    ...
    archivo = request.files["acta"]
    if archivo.filename == "":
        flash("No se seleccionó ningún archivo.", "error")
        return redirect(url_for("reuniones.ver", id=id))

    # Por ahora, solo marcamos que se subió (sin storage real)
    # En producción, guardar en S3/filesystem y crear registro en gore_documental.documento
    # reunion.instancia.acta_documento_id = documento.id
    flash("Acta recibida. (Almacenamiento pendiente de implementar)", "info")
    return redirect(url_for("reuniones.ver", id=id))


@reuniones_bp.route("/<uuid:id>/participantes", methods=["POST"])
@login_required
def agregar_participante(id):
    ...
    # Lógica de agregar participante depende del modelo de asistencia
    # Por ahora placeholder
    flash("Participante agregado. (Modelo de asistencia pendiente)", "info")
    return redirect(url_for("reuniones.ver", id=id))


@reuniones_bp.route("/<uuid:id>/participantes/<uuid:usuario_id>", methods=["POST"])
@login_required
def quitar_participante(id, usuario_id):
    """Quitar participante de la reunión."""
    if not _reuniones_habilitadas():
        return render_template("reuniones/no_disponible.html")

    reunion = Reunion.query.get_or_404(id)

    # Lógica de quitar participante depende del modelo de asistencia
    flash("Participante eliminado. (Modelo de asistencia pendiente)", "info")
    return redirect(url_for("reuniones.ver", id=id))
